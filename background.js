chrome.runtime.onInstalled.addListener(function (){
    chrome.contextMenus.create({
        title: "Edit with Inkscape online",
        id: "inkscapeUpload",
        contexts: ["image"]
    });
});
...
chrome.contextMenus.onClicked.addListener(function(info, tab){
    if (info.menuItemId === "inkscapeUpload") {
        var imgUrl = info.srcUrl;
        inkscapeUpload(imgUrl);
    }
});



function inkscapeUpload(imgUrl) {      
  	...
  	if ( (imgUrl.indexOf("http://") !=-1) || (imgUrl.indexOf("https://") !=-1)) {
  		...
  		finalurl =  "http://www.offidocs.com/edit-inkscape.php?fileurl="+ imgUrl+"&filename="+ filenamex ;
    	//alert(finalurl);
		...
		$.ajax({
    		...
    		data: formData})
        		.done(function(e){
            		finalurl =  "http://www.offidocs.com/edit-inkscape.php?filename="+ filenamex + "&fileurl=post";
        			window.open(finalurl,'_blank');
        		})
